...
import { MessageCircleCode, Lightbulb, UserCheck, Zap } from 'lucide-react';

const AsesoriaPhoneOverlay = () => {
  const [step, setStep] = useState(0);
  const [isMobile, setIsMobile] = useState(false);
  const [hydrated, setHydrated] = useState(false); // 🔐 SSR-safe

  const steps = [
    { icon: <MessageCircleCode className="w-5 h-5 text-white" />, text: 'Recibiendo tu consulta legal' },
    { icon: <Lightbulb className="w-5 h-5 text-white" />, text: 'Analizando con normativa vigente' },
    { icon: <UserCheck className="w-5 h-5 text-white animate-pulse" />, text: 'Respuesta validada por expertos' }
  ];
      ...
      >
        ...
        <div className="flex-1 p-5 space-y-4 overflow-hidden">
          <div className="text-center">
            ...
            <h2 className="text-sm font-bold text-[#10B981] mb-1">Asesoría Legal Inteligente</h2>
            <p className="text-[10px] text-gray-400">Respuestas guiadas por ley ecuatoriana</p>
          </div>
...
export default AsesoriaPhoneOverlay;
