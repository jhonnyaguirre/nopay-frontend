...
import { FileText, CheckCircle, Landmark, Zap } from 'lucide-react';

const RegularizacionAnimacionOverlay = () => {
  const [step, setStep] = useState(0);
  const [isMobile, setIsMobile] = useState(false);
  const [hydrated, setHydrated] = useState(false);

  const steps = [
    { icon: <FileText className="w-5 h-5 text-white" />, text: 'Reuniendo documentación de propiedad' },
    { icon: <CheckCircle className="w-5 h-5 text-white" />, text: 'Validando requisitos legales' },
    { icon: <Landmark className="w-5 h-5 text-white animate-pulse" />, text: 'Gestionando título de dominio' }
  ];
      ...
      >
        ...
        <div className="flex-1 p-5 space-y-4 overflow-hidden">
          <div className="text-center">
            ...
            <h2 className="text-sm font-bold text-[#10B981] mb-1">Motor de Regularización</h2>
            <p className="text-[10px] text-gray-400">Trámite legal asistido por IA</p>
          </div>
        ...
        <div className="text-center text-[10px] text-gray-500 py-2 border-t border-gray-800 bg-gray-900/50 backdrop-blur-sm">
          ...
          >
            NoPay Regularización
          </motion.span>{' '}
          · Gestión legal en Ecuador
        </div>
      </motion.div>
    </motion.div>
  );
...
export default RegularizacionAnimacionOverlay;
