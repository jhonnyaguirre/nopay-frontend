...
type MigracionPhoneOverlayProps = {
  onFinish?: () => void; // NUEVA PROP
};

const MigracionPhoneOverlay = ({ onFinish }: MigracionPhoneOverlayProps) => {
  ...
  const steps = [
    {
      ...
      text: 'Cargando datos de identidad y pasaporte'
    },
    {
      icon: <ShieldCheck className="w-5 h-5 text-white" />,
      text: 'Verificando requisitos legales'
    },
    {
      icon: <Landmark className="w-5 h-5 text-white animate-pulse" />,
      text: 'Trámite migratorio en proceso'
    }
  ];
  ...
  useEffect(() => {
    ...
    // NUEVO: Ejecutar onFinish tras completar todos los pasos
    const finalTimeout = setTimeout(() => {
      if (isMobile && onFinish) {
        onFinish();
      }
    }, 1300 * steps.length + 1000); // pequeño extra para que se vea bien
      ...
      >
        ...
        <div className="flex-1 p-5 space-y-4 overflow-hidden">
          <div className="text-center">
            ...
            <h2 className="text-sm font-bold text-[#10B981] mb-1">Motor Migratorio</h2>
...
export default MigracionPhoneOverlay;
