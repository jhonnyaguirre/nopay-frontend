...
import { FileText, CalendarCheck, Check, Zap } from 'lucide-react';

const MatriculacionPhoneOverlay = () => {
  const [step, setStep] = useState(0);
  const [isMobile, setIsMobile] = useState(false);
  const [hydrated, setHydrated] = useState(false);

  const steps = [
    { icon: <FileText className="w-5 h-5 text-white" />, text: 'Consultando valores del vehículo' },
    { icon: <CalendarCheck className="w-5 h-5 text-white" />, text: 'Agendando turno' },
    {
      icon: (
        <motion.div
      ...
      text: 'Trámite enviado con éxito'
    }
  ];
      ...
      >
        ...
        <div className="flex-1 p-5 space-y-4 overflow-hidden">
          <div className="text-center">
            <motion.span
              className="block text-xl font-black bg-gradient-to-r from-[#7F1D1D] via-[#EC4899] to-[#F59E0B] bg-[length:200%_200%] bg-clip-text text-transparent animate-gradient"
            ...
            <h2 className="text-sm font-bold text-[#10B981] mb-1">Motor de Matriculación</h2>
            <p className="text-[10px] text-gray-400">Sistema guiado por NoPay</p>
          </div>
...
export default MatriculacionPhoneOverlay;
