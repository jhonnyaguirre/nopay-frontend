...
// --------------------------------------------------------------
// SlideToActionButton: Ajustado para simetría perfecta y centrado
// --------------------------------------------------------------
...
// --------------------------------------------------------------
// RotatingText: Tu componente original optimizado para fluidez
// --------------------------------------------------------------
const RotatingText = ({
  phrases,
  initialDelay = 900,
...
// --------------------------------------------------------------
// Componente principal: El alma de NoPay potenciada
// --------------------------------------------------------------
const EliteLegalHeroFusion = () => {
  const { scrollY } = useScroll();
  const [mounted, setMounted] = useState(false);
  const [activeBenefit, setActiveBenefit] = useState(2);
  const [offsetMultiplier, setOffsetMultiplier] = useState(290); // Ajustado para mejor separación en pantallas grandes
  ...
  useEffect(() => {
    setMounted(true);
    const handleResize = () => {
      // Evita que las tarjetas se encimen en pantallas anchas
      setOffsetMultiplier(window.innerWidth >= 1280 ? 320 : window.innerWidth >= 768 ? 280 : 220);
    };
    handleResize();
  ...
  const nextBenefit = useCallback(() => setActiveBenefit((prev) => (prev + 1) % benefitCards.length), [benefitCards.length]);
  const prevBenefit = useCallback(() => setActiveBenefit((prev) => (prev - 1 + benefitCards.length) % benefitCards.length), [benefitCards.length]);
  ...
  return (
    <>
      ...
      <main className="relative w-full bg-white font-['Inter',system-ui] antialiased selection:bg-rose-500 selection:text-white">
        ...
        {/* ==================== HERO SECTION (VIBRANTE Y ENÉRGICO) ==================== */}
        <div className="relative w-full min-h-[90vh] md:min-h-screen flex flex-col overflow-hidden bg-[#020617]">
          ...
          >
            ...
            <h1 className="text-center text-5xl md:text-[5.0rem] font-black text-white leading-[1.05] mb-8 tracking-[-0.02em]">
              <div className="overflow-hidden">
                <motion.div
                  ...
                  className="flex flex-col md:flex-row md:flex-wrap justify-center items-center gap-x-4 gap-y-2"
                >
            ...
            {/* MEJORA CLAVE: Contenedor del CTA Limpio y Simétrico (Elimina la elipse vacía gigante) */}
            <motion.div
              variants={textReveal}
              initial="hidden"
              animate="visible"
            ...
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-4 mt-12 mb-12">
          ...
          {/* Conservamos intacta tu hermosa ola decorativa inferior */}
        ...
        {/* ==================== SECCIÓN BENEFICIOS (CONSERVANDO FONDO BLANCO Y COLOR) ==================== */}
        <section className="relative bg-white -mt-1 pt-10 md:pt-16 pb-24 md:pb-32 px-6 overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto">
            ...
            {/* Carrusel de beneficios optimizado espacialmente */}
            <div className="relative mt-16 md:mt-20">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="absolute left-0 md:left-4 top-1/2 z-30 -translate-y-1/2 grid h-12 w-12 place-items-center rounded-full border border-slate-200 bg-white/90 text-slate-950 shadow-lg backdrop-blur-xl transition-all duration-300 hover:border-orange-300"
                aria-label="Anterior"
              >
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.1, x: 2 }}
                whileTap={{ scale: 0.95 }}
                className="absolute right-0 md:right-4 top-1/2 z-30 -translate-y-1/2 grid h-12 w-12 place-items-center rounded-full border border-slate-200 bg-white/90 text-slate-950 shadow-lg backdrop-blur-xl transition-all duration-300 hover:border-rose-300"
                aria-label="Siguiente"
              >
                <ArrowRight className="h-5 w-5" strokeWidth={1.5} />
              </motion.button>

              <div className="mx-auto flex min-h-[410px] max-w-7xl items-center justify-center overflow-hidden px-4 md:px-16">
                  ...
                  return (
                    <motion.div
                      ...
                      animate={{
                        ...
                        scale: isActive ? 1.08 : 0.83, // Un pelín menos escala a los lados para que no compitan con la tarjeta central
                        rotate: offset * 3,
                        opacity: isActive ? 1 : 0.45, // Más contraste de opacidad para guiar el ojo al centro
                      ...
                      className="absolute w-[220px] md:w-[280px] will-change-transform"
                      style={{ backfaceVisibility: 'hidden' }}
                    >
                      <div
                        className={[
                          'group relative h-[330px] md:h-[360px] overflow-hidden rounded-[2.1rem] border bg-white p-6 md:p-7 text-center transition-all duration-500',
                          isActive
                            ? 'border-transparent shadow-[0_30px_90px_rgba(244,63,94,0.18)]' // Sombra sutil con tono de NoPay
                            : 'border-slate-200/80 shadow-[0_20px_50px_rgba(15,23,42,0.04)]',
                        ].join(' ')}
                      >
                        {isActive && (
                          <motion.div
                        ...
                        <div
                          className={[
                            'absolute inset-[2px] rounded-[2rem]', // Un borde ligeramente más grueso para el gradiente
                            isActive
                        ...
                        <div className="relative z-10 flex h-full flex-col items-center justify-center">
                          <div
                            className={[
                              'grid h-24 w-24 md:h-26 md:w-26 place-items-center rounded-full transition-all duration-500',
                              isActive
                                ? `bg-gradient-to-br ${card.accent} text-white shadow-lg`
                                : 'bg-slate-100 text-slate-700',
                            ].join(' ')}
                          >
                            {React.cloneElement(card.icon, { className: 'h-10 w-10 md:h-11 md:w-11' })}
                          </div>
                          <h3 className="mt-6 text-xl md:text-2xl font-black tracking-[-0.02em] text-slate-950">
                            {card.title}
                          </h3>
                          <p className="mt-3 max-w-[210px] text-xs md:text-sm leading-relaxed text-slate-500">
                            {card.desc}
                          </p>
                          <div
