...
const PROVINCIAS_PLACA_VALIDAS = new Set([
  'A',
  'B',
  'C',
  'E',
...
const GlobalLoader = memo(function GlobalLoader({ message }: { message: string }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
...
const InputBase = memo(function InputBase({
  className = '',
  icon,
  right,
  ...props
...
const SelectBase = memo(function SelectBase({
  className = '',
  ...props
...
const SummaryMiniCard = memo(function SummaryMiniCard({
  label,
  value,
  tone = 'default',
}: {
...
// ======================== MODAL MEJORADO ========================
const MultasModal = memo(function MultasModal({
  isOpen,
  onClose,
  multas,
  placa,
  ...
  const totalMonto = useMemo(() => {
    return multas.reduce((acc, m) => {
      const val = parseFloat(m.total?.replace(/[^0-9.-]/g, '') || '0') / 100;
      return acc + (isNaN(val) ? 0 : val);
    }, 0);
  }, [multas]);

  const totalConNoPay = useMemo(
    () => totalMultas * valorServicio,
    [totalMultas, valorServicio]
  );
    ...
    >
      ...
      <div className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur-xl md:px-7">
        <div className="flex items-start justify-between gap-4">
            ...
            <div>
              <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                  Diagnóstico vehicular NoPay
                </span>
              </div>

              <h3
                id="dialog-title"
                className="text-2xl font-black tracking-[-0.04em] text-slate-950 md:text-3xl"
              >
              ...
              <p className="mt-1 text-sm text-slate-500">
                Placa consultada:{' '}
                <span className="rounded-md bg-slate-100 px-2 py-0.5 font-mono font-black tracking-wider text-slate-800">
                  {placa.toUpperCase()}
                </span>
              </p>
            </div>
          ...
          <button
            onClick={onClose}
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-slate-200 bg-white text-slate-400 shadow-sm transition-all hover:bg-slate-950 hover:text-white focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2"
      ...
      <div className="relative flex-1 overflow-y-auto bg-[#FBFBFD] px-5 py-5 md:px-7 scrollbar-thin scrollbar-thumb-slate-300">
        {totalMultas > 0 ? (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="rounded-[1.4rem] border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-center gap-2 text-slate-500">
                  ...
                  <p className="text-[11px] font-black uppercase tracking-[0.17em]">
                    Multas detectadas
                  </p>
                </div>
              ...
              {ahorro > 0 && (
                <div className="rounded-[1.4rem] border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm font-semibold text-emerald-700">
                    Hasta {porcentajeAhorro}% menos.
                  </p>
                </div>
              )}
            </div>

            {/* CARD PRINCIPAL NOPAY MÁS LIMPIA */}
              ...
              <div className="relative p-5 text-white md:p-6">
                ...
                <div className="relative flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <h4 className="mt-1 text-2xl font-black tracking-[-0.04em] md:text-3xl">
                        Con NoPay pagas {formatCurrency(totalConNoPay)} + IVA
                      </h4>

                      <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/65">
                        Revisamos tus multas, ordenamos la información y te guiamos para
                        continuar con mayor claridad.
                      </p>
                    </div>
                  </div>
                ...
                <div className="relative mt-5 grid gap-3 md:grid-cols-3">
                  {[
                    ...
                    'Proceso claro',
                    'Pago fijo por infracción',
                  ].map((item, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm text-white/85"
                    >
            ...
            {/* INDICADOR VISIBLE DESDE EL PRIMER PANTALLAZO */}
            <button
              type="button"
              onClick={() => {
                const detalle = document.getElementById('detalle-multas');
                detalle?.scrollIntoView({ behavior: 'smooth', block: 'start' });
              }}
              className="group flex w-full items-center justify-between rounded-[1.3rem] border border-rose-100 bg-white px-4 py-3 text-left shadow-sm transition-all hover:border-rose-200 hover:shadow-md"
            >
              <div className="flex items-center gap-3">
                <div className="grid h-9 w-9 place-items-center rounded-full bg-rose-50 text-rose-600">
                  <ChevronRight className="h-4 w-4 rotate-90 transition-transform group-hover:translate-y-0.5" />
                </div>

                <div>
                  <p className="text-sm font-black text-slate-900">
                    Ver respaldo de la multa
                  </p>
                  <p className="text-xs text-slate-500">
                    Abajo está el detalle consultado: infracción, fecha, ente y valor.
                  </p>
                </div>
              </div>

              <span className="hidden rounded-full bg-slate-100 px-3 py-1 text-[11px] font-bold text-slate-500 md:inline-flex">
                Deslizar abajo
              </span>
            </button>
              ...
              <div className="flex flex-col gap-2 border-b border-slate-200 bg-white px-5 py-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-sm font-black text-slate-950">
                    Detalle de multas encontradas
                  </p>
                  <p className="text-xs text-slate-500">
                    Información de respaldo para revisar antes de continuar.
                  </p>
                </div>

                <div className="inline-flex w-fit items-center gap-2 rounded-full bg-slate-50 px-3 py-1 text-xs font-bold text-slate-500 ring-1 ring-slate-200">
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="sticky top-0 z-10 bg-slate-50/95 backdrop-blur">
                      <tr className="border-b border-slate-200 text-left text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">
                        <th className="px-5 py-3">N° Infracción</th>
                        <th className="px-5 py-3">Ente</th>
                    ...
                    <tbody className="divide-y divide-slate-100 bg-white">
                      {multas.map((multa, idx) => (
                        <tr key={idx} className="transition-colors hover:bg-rose-50/40">
                          <td className="px-5 py-4 font-mono text-sm font-bold text-slate-800">
                            {multa.id_factura || multa.secuencia_1}
                          </td>

                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2">
                              <Building2 className="h-3.5 w-3.5 text-slate-400" />
                              <span className="text-slate-600">{multa.ente}</span>
                            </div>
                          </td>
              ...
              <div className="flex gap-3">
                ...
                <p className="text-sm leading-relaxed text-slate-600">
                  <strong className="font-black text-slate-950">
                    ¿Por qué hacerlo con NoPay?
                  </strong>{' '}
                  Te damos una lectura clara del problema y te ayudamos a avanzar con
                  un proceso más simple, guiado y seguro.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex min-h-[520px] flex-col items-center justify-center text-center">
            <div className="relative mb-7">
              <div className="absolute inset-0 rounded-full bg-emerald-300/25 blur-2xl" />
              <div className="relative grid h-24 w-24 place-items-center rounded-full bg-emerald-50 ring-1 ring-emerald-100">
            ...
            <h4 className="mt-5 text-3xl font-black tracking-[-0.04em] text-slate-950 md:text-4xl">
              No encontramos multas pendientes
            </h4>

            <p className="mt-3 max-w-xl text-base leading-relaxed text-slate-500">
              Tu vehículo no registra multas en esta consulta. Si tienes una citación
              física, puedes cargarla para que NoPay la revise.
            </p>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <p className="text-base font-black text-slate-950">
                    ¿Tienes una multa física?
                  </p>

                  <p className="mt-1 text-sm leading-relaxed text-slate-500">
                    Puedes cargar la citación en el siguiente paso para asociarla a tu
                    vehículo e iniciar el proceso correspondiente.
                  </p>

                  <button className="mt-4 inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-white transition-all hover:bg-rose-600">
                    Cargar multa física
      ...
      <div className="sticky bottom-0 z-30 border-t border-slate-200 bg-white/95 px-5 py-4 backdrop-blur-xl md:px-7">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <p className="text-xs leading-relaxed text-slate-500">
            Esta información es referencial y será validada durante el proceso.
          </p>

          <button
            onClick={onClose}
            className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 py-3 text-sm font-black text-white shadow-lg transition-all hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2"
          >
...
const AdvancedForm = () => {
  ...
  const formatDateToInput = useCallback((date: Date): string => {
  ...
  const validarPlacaEcuador = useCallback((placa: string) => {
    const regex = /^[A-Z]{3}-?\d{3,4}$/;
    const placaNormalizada = placa.trim().toUpperCase();
    if (!regex.test(placaNormalizada)) return false;
    return PROVINCIAS_PLACA_VALIDAS.has(placaNormalizada.charAt(0));
  }, []);

  const validarCedulaEcuador = useCallback((cedula: string) => {
  ...
  const cedulaValida = useMemo(() => validarCedulaEcuador(cedula), [cedula, validarCedulaEcuador]);
  const placaValida = useMemo(
    () => validarPlacaEcuador(vehicleData.placa || ''),
    [vehicleData.placa, validarPlacaEcuador]
  );
  ...
  useEffect(() => {
    setWizardData(SessionWizardData.obtener());
  }, []);
  ...
  useEffect(() => {
    const onStorageChange = (e: StorageEvent) => {
      if (e.key === 'authTokenWizard') validateTokenAndAuth();
    };
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      const cedulaObjetivo = cedula ? cedula.trim() : wizardData?.cedula?.trim();

      const debeConsultar = omitValidacionInicial || (cedula.length === 10 && cedulaValida);
      if (!debeConsultar || !cedulaObjetivo) {
        setShowUserForm(false);
        if (cedula.length > 0 && cedula.length === 10 && !cedulaValida) {
          setErrorMsg('La cédula no es válida. Verifica los dígitos.');
        }
        return;
      }
      ...
      try {
        ...
        try {
          ...
          console.error('❌ Error de red:', err.message || err);
          if (!emailRegex.test(cedulaObjetivo)) setCedula('');
        ...
        if (wizardData?.cedula) setCedula(wizardData.cedula.trim());

        let emailFinal = '';
        if (data.email) emailFinal = data.email;
        else if (wizardData?.cedula) emailFinal = wizardData.cedula;

        if (wizardData && data.email != null) setCedula(wizardData.cedula);

        setCedula(data.cedula || cedulaObjetivo);

        SessionWizardData.guardar({
          cedula: data.cedula || cedulaObjetivo,
  ...
  useEffect(() => {
    try {
      ...
      if (wizardData) {
        ...
        setEmail(wizardData.email || wizardData.cedula);
  ...
  const mapearTipo = useCallback((clase: string): string => {
    if (!clase) return '';
  ...
  const buscarPlaca = useCallback(async (placaConsultar?: string) => {
    ...
    try {
      const token = getWizardToken();
      const url = `${CITACIONES_API_URL}?tipo=PLA&identificacion=${encodeURIComponent(
        placaAUsar
      )}&estado=pendientes`;
      ...
      if (data.meta?.error) {
        ...
        setVehicleData((prev) => ({
          ...prev,
          placa: placaAUsar,
          marca: cab.Marca || prev.marca,
          modelo: cab.Modelo || prev.modelo,
          color: cab.Color ? cab.Color.toLowerCase() : prev.color,
          año: cab['Año'] || cab['Año de Matrícula'] || prev.año,
  ...
  }, [mapearTipo, validarPlacaEcuador, vehicleData.placa]);

  const handleFormSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();

    if (!userData.telefono) {
      setAuthError('El número de teléfono es obligatorio para contactarte.');
      return;
    }

    if (!userData.email) {
      setAuthError('El correo electrónico es obligatorio para comunicaciones legales.');
      return;
    }

    if (!vehicleData.placa) {
      setAuthError('La placa del vehículo es obligatoria para continuar.');
      return;
    }
  ...
  }, [cedula, multas, router, userData, vehicleData]);
  ...
  const puedeContinuar = useMemo(
    () =>
      ...
      !!userData.email &&
      !!vehicleData.placa,
    [showUserForm, showVehicleForm, userData.email, userData.telefono, vehicleData.placa]
  );

  const progreso = useMemo(
    () => (Number(showUserForm) + Number(showVehicleForm && !!vehicleData.placa)) * 50,
    [showUserForm, showVehicleForm, vehicleData.placa]
  );
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-100 via-cyan-50 to-blue-100 p-4">
        <motion.div
          ...
          className="w-full max-w-md rounded-[28px] border border-slate-200/80 bg-white/95 p-8 text-center shadow-2xl"
        >
          ...
          <button
            ...
            className="mt-6 rounded-2xl bg-gradient-to-r from-cyan-600 to-blue-700 px-6 py-3 font-semibold text-white shadow-lg transition hover:translate-y-[-1px] hover:shadow-xl"
          >
            Reintentar
          </button>
        </motion.div>
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showCedulaDuplicada && (
          <Notification
            ...
            onClose={() => setShowCedulaDuplicada(false)}
          />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {consultandoPlaca && (
          <GlobalLoader message="Buscando información del vehículo y multas..." />
        )}
      </AnimatePresence>

      <AnimatePresence>
        {submitting && (
          <GlobalLoader message="Guardando información, por favor espera..." />
        )}
      </AnimatePresence>
        ...
        <main className="mx-auto max-w-7xl py-8">
          ...
          <div className="mb-6 rounded-[28px] border border-slate-200/80 bg-white/90 p-5 shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
                  Verificaremos tu información personal y la de tu vehículo para
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1">
                  <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-cyan-600 text-sm font-bold text-white">
                    1
                  </span>
                  <span className="text-sm font-medium text-slate-700">Datos</span>
                </div>
                <div className="h-px w-6 bg-slate-300" />
                <div
                  className={`flex items-center gap-1 ${progreso >= 50 ? 'opacity-100' : 'opacity-40'}`}
                >
                  <span
                ...
                <div className="h-px w-6 bg-slate-300" />
                <div
                  className={`flex items-center gap-1 ${progreso >= 100 ? 'opacity-100' : 'opacity-40'}`}
                >
                  <span
                    className={`inline-flex h-8 w-8 items-center justify-center rounded-full ${
                      progreso >= 100
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="space-y-6">
              <div >
			  <SectionCard
				title="Iniciamos con la Impugnación de tús multas"
			  ...
			  >
                <form onSubmit={handleFormSubmit} className="space-y-8 pb-24">
                  <div className="hidden grid grid-cols-1 gap-6">
                    ...
                    {/* Security badge */}
                    <div className="mt-2 mb-4 flex items-center gap-2 rounded-full bg-cyan-50 px-3 py-1.5 text-xs text-cyan-700 w-fit border border-cyan-100">
                  ...
                  <AnimatePresence>
                    {showUserForm && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="hidden rounded-[24px] border border-slate-200 bg-slate-50/70 p-5 sm:p-6"
                      >
                        <div className="mb-6 flex items-start gap-4">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-600">
                              Revisa y completa tus datos antes de continuar.
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                          ...
                          <div className="md:col-span-2">
                            <Field
                              ...
                              required
                              hint="Este campo es informativo y viene asociado a tu sesión."
                            >
                              <InputBase
                                type="email"
                    ...
                    <div className="rounded-[24px] border border-slate-200 bg-white p-5 sm:p-6">
                      <div className="mb-6 flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-600">
                            Selecciona uno de tus vehículos o agrega uno nuevo.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                        {vehiculosUsuario.map((vehiculo, index) => (
                          <motion.div
                            ...
                            whileHover={{ y: -3 }}
                            className={cn(
                              'relative overflow-hidden rounded-[24px] border p-5 shadow-sm transition cursor-pointer',
                            ...
                            onClick={() => {
                              ...
                              buscarPlaca(vehiculo.placa);
                            }}
                          >
                            <div className="mb-4 flex items-start justify-between gap-3">
                              <div>
                                <h4 className="text-xl font-bold tracking-wider text-slate-900">
                                  {vehiculo.placa}
                                </h4>
                                <span
                                  className={cn(
                              ...
                              {vehiculoSeleccionado === vehiculo.placa && (
                                <div className="inline-flex items-center rounded-full border border-cyan-200 bg-white px-2.5 py-1 text-xs font-semibold text-cyan-700">
                                ...
                                <div>
                                  ...
                                  <div className="mt-1 flex items-center gap-2">
                                    <span
                                      ...
                                      style={{
                                        backgroundColor: getColorCode(vehiculo.color),
                                      }}
                                    />
                      ...
                      <div className="mt-8 flex justify-center">
                        <button
                          ...
                          className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-600 to-blue-700 px-6 py-3 text-sm font-semibold text-white shadow-lg transition hover:translate-y-[-1px] hover:shadow-xl"
                        >
                          <PlusCircle className="h-4 w-4" />
                          Agregar otro vehículo
                          <span className="text-xs opacity-80">(si no ves el tuyo)</span>
                        </button>
                      </div>
                    </div>
                  )}
                      ...
                      >
                        <div className="mb-6 flex items-start gap-4">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-600">
                              Consulta la placa y completa los datos faltantes si es necesario.
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-6">
                          <div className="space-y-6">
                            <Field
                              label="Placa"
                              hint="Formato ecuatoriano: tres letras seguidas de tres o cuatro dígitos (ej: ABC1234)."
                              error={vehicleData.placa.length >= 6 && !placaValida ? 'Formato de placa inválido.' : errorPlaca}
                            >
                              <div className="flex gap-3">
                                <InputBase
                                  type="text"
                                  ...
                                  onChange={(e) =>
                                    setVehicleData({ ...vehicleData, placa: e.target.value.toUpperCase() })
                                  }
                                ...
                                <button
                                  type="button"
                                  onClick={() => buscarPlaca()}
                                  disabled={!placaValida || consultandoPlaca}
                                  className={cn(
                                    'inline-flex items-center justify-center rounded-2xl px-5 py-3 shadow-lg transition',
                                    placaValida && !consultandoPlaca
                                      ? 'bg-gradient-to-r from-cyan-600 to-blue-700 text-white hover:translate-y-[-1px] hover:shadow-xl'
                  ...
                  {errorPlaca && (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="rounded-2xl border border-red-200 bg-red-50/70 p-4 text-sm font-medium text-red-700 flex items-center gap-2"
                    >
                  ...
                  {/* Botón sticky dentro del formulario */}
                  <div className="sticky bottom-0 -mb-24 mt-8 pt-4 pb-6 bg-white/95 backdrop-blur-sm border-t border-slate-200 rounded-b-2xl z-20">
                    <div className="flex justify-end">
                      <button
                        type="submit"
                        disabled={!puedeContinuar || submitting}
                        className={cn(
                          'inline-flex items-center gap-2 rounded-2xl px-8 py-3 font-semibold shadow-lg transition',
                          puedeContinuar && !submitting
                            ? 'bg-gradient-to-r from-cyan-600 to-blue-700 text-white hover:translate-y-[-1px] hover:shadow-xl'
                            : 'cursor-not-allowed bg-slate-200 text-slate-400'
                        )}
                      >
            ...
            <aside className="space-y-6">
              <div className="rounded-[28px] border border-slate-200/80 bg-white/90 p-5 shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl">
                ...
                <div className="mt-5 space-y-4">
                  <SummaryMiniCard
                    label="Vehículos"
                    value={vehiculosUsuario.length}
                    tone="default"
                  />
                  <SummaryMiniCard
                    label="Multas"
                    value={multas.length}
                    tone={multas.length > 0 ? 'info' : 'default'}
                  />
                  <SummaryMiniCard
                    label="Estado"
                    value={puedeContinuar ? 'Listo' : 'Pendiente'}
                    tone={puedeContinuar ? 'success' : 'default'}
                  />
                </div>
              </div>
              ...
              <div className="rounded-[28px] border border-slate-200/80 bg-white/90 p-5 shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-slate-900">Garantía de seguridad legal</h3>
                    <p className="mt-1 text-xs text-slate-500">
                      Todos los datos están protegidos bajo estándares de confidencialidad.
                      No compartimos tu información con terceros sin tu autorización expresa.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[28px] border border-slate-200/80 bg-white/90 p-5 shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl">
                <h3 className="text-lg font-bold text-slate-900">Consejo</h3>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  Verifica cuidadosamente tu teléfono, tu correo y la placa del vehículo.
                  Esa será usada en el siguiente paso del proceso.
                </p>
              </div>
            </aside>
          </div>
