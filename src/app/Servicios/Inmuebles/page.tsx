...
import RegularizacionAnimacionOverlay from 'app/resources/animaciones/Inmuebles/page';

export default function RegularizacionPropiedadesPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#7F1D1D] via-[#EC4899] to-[#F59E0B] text-white relative overflow-hidden text-[90%]">
      ...
      {isMobile && showOverlay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-[#7F1D1D] via-[#EC4899] to-[#F59E0B]">
          <div className="w-full h-full flex items-center justify-center">
            <RegularizacionAnimacionOverlay />
          </div>
        </div>
      )}
      ...
      <section className={`relative z-10 max-w-7xl mx-auto px-6 py-16 lg:py-24 flex flex-col-reverse lg:flex-row items-center gap-12 ${isMobile && showOverlay ? 'hidden' : ''}`}>        <div className="w-full lg:w-1/2">
          ...
          >
            {[
              { icon: <ScrollText className="h-5 w-5" />, text: "Títulos de propiedad validados" },
              { icon: <FileText className="h-5 w-5" />, text: "Trámite con municipio y catastros" },
              { icon: <CheckCircle className="h-5 w-5" />, text: "Aplica a programas de legalización urbana" }
            ].map((item, i) => (
        ...
        >
          <div className="relative w-full h-auto">
            {!isMobile && <RegularizacionAnimacionOverlay />}
          </div>
        </motion.div>
      </section>
            ...
            {[
              {
                ...
                description: "Predios, croquis, certificados municipales o de posesión.",
                icon: <FileText className="h-8 w-8 text-[#EC4899]" />
              },
              {
                step: "2",
                title: "Validamos requisitos legales",
                description: "Verificamos cumplimiento ante municipio, MIDUVI y catastro.",
                icon: <CheckCircle className="h-8 w-8 text-[#F59E0B]" />
              },
              {
                step: "3",
                title: "Gestionamos tu título",
                description: "Tramitamos la escritura o título de dominio con registro público.",
      ...
      <section className="py-16 bg-gradient-to-r from-[#7F1D1D] to-[#EC4899] text-[90%]">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <motion.h3
            ...
            transition={{ duration: 0.6 }}
          >
            ¿Listo para legalizar tu propiedad?
          </motion.h3>
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            whileInView={{ opacity: 1, y: 0 }}
