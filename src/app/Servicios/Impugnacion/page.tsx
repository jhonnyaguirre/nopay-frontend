...
import { ShieldCheck, Zap, Gavel, Check, ChevronRight, FileText } from 'lucide-react';
      ...
      <section className={`relative z-10 max-w-7xl mx-auto px-6 py-16 lg:py-24 lg:flex lg:items-center lg:gap-12 ${isMobile && showOverlay ? 'hidden' : ''}`}>
        <div className="max-w-2xl mb-16 lg:mb-0">
          ...
          >
            {[
              ...
              { icon: <Check className="h-5 w-5" />, text: "92% de éxito" },
              { icon: <Gavel className="h-5 w-5" />, text: "Sin trámites presenciales" }
            ].map((item, i) => (
        ...
        >
          <div className="relative w-full h-auto">
            {!isMobile && <ImpugnacionPhoneOverlay />}
          </div>
        </motion.div>
      </section>

      {/* Sección Cómo Funciona - Ahora con fondo claro */}
            ...
            {[
              {
                step: "1",
                title: "Sube tu multa",
                description: "Toma foto o carga el documento de tu multa de tránsito",
                icon: <FileText className="h-8 w-8 text-[#EC4899]" />
              },
              {
                step: "2",
                title: "Analizamos tu caso",
                description: "Nuestra IA detecta posibles errores y argumentos legales",
                icon: <Zap className="h-8 w-8 text-[#F59E0B]" />
              },
              {
                step: "3",
                title: "Presentamos tu defensa",
                description: "Generamos y enviamos tu recurso de impugnación",
