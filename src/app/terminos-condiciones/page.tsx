...
import { FileText, ShieldCheck, Scale, Copyright, AlertTriangle, BookOpen, Mail, CheckCircle } from 'lucide-react';
...
export default function TerminosCondiciones() {
  ...
  const termsItems = [
    { title: "1. Objeto del Servicio", text: "NoPay Legal ofrece un servicio tecnológico para automatizar recursos legales administrativos. No garantiza una resolución favorable ni sustituye el patrocinio de un abogado.", icon: <FileText className="text-pink-600" /> },
    { title: "2. Limitación de Responsabilidad", text: "No garantizamos decisiones favorables de las autoridades. No nos responsabilizamos por información errónea proporcionada por el usuario. No ofrecemos asesoría jurídica directa ni representación legal.", icon: <AlertTriangle className="text-pink-600" /> },
    { title: "3. Fuerza Mayor", text: "No nos responsabilizamos por fallos causados por fuerza mayor, conforme al artículo 30 del Código Civil del Ecuador.", icon: <Scale className="text-pink-600" /> },
    { title: "4. Propiedad Intelectual", text: "Todo el contenido y tecnología de la plataforma es propiedad exclusiva de NoPay Legal. Su uso no autorizado está prohibido.", icon: <Copyright className="text-pink-600" /> },
    { title: "5. Protección de Datos", text: "Los datos se gestionan según la Ley Orgánica de Protección de Datos Personales. El usuario puede ejercer sus derechos cuando lo desee.", icon: <ShieldCheck className="text-pink-600" /> },
    { title: "6. Jurisdicción", text: "Las disputas se resolverán bajo la jurisdicción del cantón Cuenca, Ecuador.", icon: <BookOpen className="text-pink-600" /> },
    { title: "7. Modificaciones", text: "NoPay Legal puede actualizar estos términos en cualquier momento. Revise esta sección periódicamente.", icon: <Mail className="text-pink-600" /> }
  ];
      ...
      <div className="container mx-auto px-4 sm:px-6 py-8 md:py-12 lg:py-16 relative z-10">
        ...
        >
          <div className="flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-12 items-stretch">
            {/* Panel izquierdo - Resumen de términos */}
            <motion.div
            ...
            >
              <div className="bg-white p-6 sm:p-8 rounded-2xl md:rounded-3xl shadow-xl border border-gray-100 h-full flex flex-col justify-between relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  ...
                  >
                    <p className="text-center text-xs md:text-sm text-pink-700 font-medium">
                      <span className="inline-block bg-white px-2 py-1 rounded-md shadow-sm">📄 Documento Legal</span> vinculante según normativa ecuatoriana
                    </p>
                  </motion.div>
                </div>
                ...
                <div className="mt-6 md:mt-8 space-y-4 md:space-y-5 relative z-10">
                  {[
                    { icon: Scale, label: 'Marco Legal', desc: 'Código Civil y Ley de Comercio Electrónico' },
                    { icon: ShieldCheck, label: 'Protección de Datos', desc: 'Ley Orgánica de Protección de Datos' },
                    { icon: AlertTriangle, label: 'Limitaciones', desc: 'No representamos asesoría legal directa' },
                    { icon: Copyright, label: 'Propiedad', desc: 'Contenido exclusivo de NoPay Legal' },
                  ].map((f, i) => (
                    <motion.div
                      key={i}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="font-medium text-sm md:text-base text-gray-800">
                      ...
                      <span className="block text-xs text-gray-600 mt-1">Última actualización: agosto 2025</span>
                    </p>
                  </div>
                  <div className="flex gap-3">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      className="text-xs md:text-sm bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-300 text-gray-800 font-medium hover:bg-gray-50 transition-colors"
                    >
                      Cancelar
                    </motion.button>
                    <motion.button
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.98 }}
                      className="text-xs md:text-sm bg-gradient-to-br from-pink-600 to-pink-500 px-4 py-2 rounded-lg shadow-sm text-white font-medium hover:from-pink-500 hover:to-pink-400 transition-all"
                    >
                      Aceptar Términos
                    </motion.button>
                  </div>
