      ...
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">
            Sólo necesitamos un poco de información
          </h1>
          <p className="mt-1 text-sm text-slate-600">
            Completa cada etapa de forma clara y ordenada.
          </p>
        </div>
                  ...
                  <div className="relative mb-3">
                    <div
                      className={cn(
                        ...
                        isCompleted &&
                          "border-emerald-200 bg-emerald-500 text-white shadow-lg shadow-emerald-200/60",
                        isActive &&
                          "border-cyan-200 bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-200/70 ring-4 ring-cyan-100",
                        isPending &&
                    ...
                    <div className="h-[3px] w-full rounded-full bg-slate-200">
                      <div
                        className={cn(
                          ...
                          step > item.id
                            ? "w-full bg-gradient-to-r from-emerald-400 to-cyan-500"
                            : "w-0 bg-transparent"
                        )}
                      />
                    </div>
              ...
              <div key={item.id} className="flex flex-col items-center gap-2">
                <div
                  className={cn(
                    ...
                    isActive &&
                      "border-cyan-200 bg-gradient-to-br from-cyan-500 to-blue-600 text-white ring-2 ring-cyan-100",
                    isPending &&
                      "border-slate-200 bg-slate-50 text-slate-400"
                  )}
                >
...
const ImpugnacionWizard = () => {
  ...
  // Estado para la animación de seguridad en paso 7
  const [securityStage, setSecurityStage] = useState(-1); // -1 = no iniciada
  ...
  const showTemporaryNotification = (message: string, duration = 5000) => {
  ...
  useEffect(() => {
    ...
    if (wizardData) {
      ...
      setNombreParam(
        `${wizardData.nombres || ""} ${wizardData.apellidos || ""}`.trim()
      );
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      if (document.visibilityState === "visible") validateTokenAndAuth();
    };
  ...
  useEffect(() => {
    const validarJWT = async () => {
      ...
      if (!esValido) {
        showTemporaryNotification(
          "Tu sesión ha expirado o es inválida. Serás redirigido."
        );
        setTimeout(() => {
  ...
  function generarPromptIAOptimista({
    ...
    prompt += `- Fecha citación: ${fechaCitacion} (Días transcurridos: ${Math.floor(
      (new Date().getTime() - new Date(fechaCitacion).getTime()) /
        (1000 * 60 * 60 * 24)
    )})\n`;
    ...
    archivos.forEach((file: FileWithPreview, idx: number) => {
      ...
      if (ocrResults[idx]) {
        ...
        if (ocrResults[idx].extractedData) {
          prompt += `   ▸ Datos estructurados: ${JSON.stringify(
            ocrResults[idx].extractedData
          )}\n`;
        }
      }
    });
  ...
  useEffect(() => {
    if (step === 5) {
      ...
      const vehiculoDescripcion = vehiculoObj
        ? vehiculoObj.descripcion
        : "No seleccionado";
      ...
      const fetchIA = async () => {
        try {
          const token = getWizardToken();
          const res = await fetch(`${API_BASE_URL}/nopaychatServices`, {
            method: "POST",
            headers: {
  ...
  }, [step, nombreParam, cedula, formData, vehiculosUsuario]);

  useEffect(() => {
    if (respuestaIA) {
      Promise.resolve(marked.parse(respuestaIA)).then((result) => {
  ...
  useEffect(() => {
    const fetchMultasVehiculo = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/vehiculos/${formData.vehiculo}/multas`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let file of rawFiles) {
      ...
      if (!isImage && !isPdf) {
        localErrors.push(`${file.name}: solo imágenes o PDF.`);
        continue;
      }
      ...
      if (file.size > MAX_SIZE) {
        if (isImage) {
          try {
            ...
            localErrors.push(`${file.name}: ${err.message}`);
            continue;
          }
        } else {
          localErrors.push(`${file.name}: supera 5 MB y no es imagen.`);
          continue;
        }
      }
    ...
    if (localErrors.length > 0) {
      showTemporaryNotification(localErrors.join(" "));
    }
    ...
    try {
      ...
      showTemporaryNotification("Error procesando archivos.");
    } finally {
      setIsProcessing(false);
  ...
  const validateStep = (stepNumber: number): boolean => {
    ...
    switch (stepNumber) {
      ...
      case 3:
        if (!(window as any).todosDocumentosCompletos) {
      ...
      case 4:
        if (!formData.vehiculo) {
          newErrors.vehiculo = "Seleccione un vehículo.";
        }

        if (!formData.numeroCitacion?.trim()) {
          newErrors.numeroCitacion = "Ingrese o seleccione el número de citación.";
        }
        ...
        if (!formData.fechaCitacion?.trim()) {
          ...
          if (!fechaRegex.test(formData.fechaCitacion)) {
            newErrors.fechaCitacion = "Formato de fecha inválido. Use AAAA-MM-DD.";
          } else {
            const dateObj = new Date(formData.fechaCitacion);
            if (isNaN(dateObj.getTime())) {
              newErrors.fechaCitacion = "Fecha no válida.";
            }
          }
        }

        // Si no hay multas, la carga de imágenes es obligatoria
        if (multasVehiculo.length === 0 && formData.archivos.length === 0) {
          newErrors.archivos = "Debes cargar al menos una imagen de la citación porque no hay multas registradas para este vehículo.";
        }
        break;
      ...
      case 5:
        ...
        if (!formData.aceptaTerminosUso) {
          newErrors.aceptaTerminosUso =
  ...
  const validateAllData = (): { isValid: boolean; errors: Record<string, string>; firstErrorStep: number | null } => {
    const allErrors: Record<string, string> = {};
    let firstErrorStep: number | null = null;

    // Step 1
    //if (!formData.direccion) allErrors.direccion = "La dirección es requerida.";
    ...
    if (Object.keys(allErrors).some(k => ["direccion", "provincia", "ciudad"].includes(k)) && firstErrorStep === null) firstErrorStep = 1;
    ...
    // Step 3
    if (!(window as any).todosDocumentosCompletos) {
      allErrors.archivos = "Debes subir todos los documentos requeridos.";
    ...
    if (!formData.fechaCitacion?.trim()) {
      allErrors.fechaCitacion = "Ingrese o seleccione la fecha de citación.";
    } else {
      const fechaRegex = /^\d{4}-\d{2}-\d{2}$/;
      if (!fechaRegex.test(formData.fechaCitacion)) {
        allErrors.fechaCitacion = "Formato de fecha inválido. Use AAAA-MM-DD.";
      } else {
        const dateObj = new Date(formData.fechaCitacion);
        if (isNaN(dateObj.getTime())) {
          allErrors.fechaCitacion = "Fecha no válida.";
        }
      }
    }
    // Si no hay multas, los archivos son obligatorios
    if (multasVehiculo.length === 0 && formData.archivos.length === 0) {
      allErrors.archivos = "Debes cargar al menos una imagen de la citación porque no hay multas registradas para este vehículo.";
    ...
    // Validación del campo "ente"
    const enteValue = multaSeleccionada?.ente || formData.agencia;
    if (!enteValue) {
      allErrors.agencia = "No se pudo determinar la entidad emisora. Seleccione una agencia o una multa existente.";
  ...
  const handleFinalSubmit = async () => {
    ...
    if (!isValid) {
      showTemporaryNotification(
        "Faltan datos requeridos. Por favor completa los campos marcados en rojo."
      );
    ...
    try {
      ...
      if (!fechaNorm) {
        setErrors((prev) => ({
          ...prev,
          fechaCitacion: "Formato inválido. Use AAAA-MM-DD o DD/MM/AAAA.",
        }));
        setStep(4);
        return;
      }
      ...
      const detalleJson = [{
        ...
        ente: multaSeleccionada?.ente || formData.agencia || "",
        secuencia_estado: 1,
      }];
      ...
      if (!res.ok) {
        ...
        showTemporaryNotification(customError);
        return;
      }

      const descripcionVehiculo =
        vehiculosUsuario.find((v) => v.secuencial.toString() === formData.vehiculo)
          ?.descripcion || "";
  ...
  // Efecto para iniciar la animación de seguridad en paso 7
  useEffect(() => {
    if (step === 7 && securityStage === -1) {
      // Inicia la animación
      setSecurityStage(0);
    }
  }, [step, securityStage]);

  // Avance automático de los pasos de seguridad
  useEffect(() => {
    if (securityStage >= 0 && securityStage < securityMessages.length) {
      ...
      // Si es el último paso, después de 1.2s ejecutamos la redirección
      if (securityStage === securityMessages.length - 1) {
  ...
  }, [securityStage, securityMessages.length]);
        ...
        <main className="mx-auto flex min-h-[70vh] max-w-7xl items-center justify-center px-4 py-10">
          <div className="rounded-[28px] border border-slate-200/80 bg-white/90 px-8 py-10 text-center shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Estamos preparando tu entorno seguro.
            </p>
          </div>
        </main>
        <Footer />
  ...
  const hayMultas = multasVehiculo.length > 0;

  // Función para hacer scroll al panel de multas
  const scrollToMultas = () => {
    if (multasPanelRef.current) {
      multasPanelRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
    }
  };
              ...
              <AnimatePresence mode="wait">
                {step === 1 && (
                  <motion.div key="step-1" {...pageTransition}>
                    <SectionCard
                      ...
                      description="Ingresa la dirección donde ocurrió el hecho. Hemos simplificado este paso para hacerlo más directo y menos cansado visualmente."
                      ...
                      <div className="space-y-6">
                        ...
                        >
                          <TextInput
                            ...
                            placeholder="Ejemplo: Av. Solano y Av. 12 de Abril"
                            error={errors.direccion}
                          />
                        </Field>
						</div>
                ...
                {step === 2 && (
                  <motion.div key="step-2" {...pageTransition}>
                    <SectionCard
                      title="Detalles principales de la multa"
                      description="Selecciona cómo fue emitida la multa y la entidad que la generó. El diseño está pensado para reducir ruido visual y facilitar la lectura."
                      icon={<Radar className="h-7 w-7" />}
                    >
                      <div className="space-y-8">
                        <Field
                          label="¿Cómo fue multado?"
                          error={errors.tipoMulta}
                          icon={<AlertTriangle className="h-4 w-4" />}
                        >
                          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                            {["Radar Móvil", "Radar Fijo", "Agente Tránsito", "Otro"].map(
                              (tipo) => (
                                <button
                                  key={tipo}
                                  type="button"
                                  onClick={() =>
                                    setFormData({ ...formData, tipoMulta: tipo })
                                  }
                                  className={cn(
                                    "rounded-2xl border px-4 py-4 text-left transition",
                                    formData.tipoMulta === tipo
                        ...
                        >
                          <SelectInput
                            ...
                            onChange={(value) =>
                              setFormData({ ...formData, agencia: value })
                            }
                            error={errors.agencia}
                          >
                ...
                {step === 3 && (
                  <motion.div key="step-3" {...pageTransition}>
                    <SectionCard
                      title="Documentos requeridos"
                      description="En esta sección puedes completar la carga documental obligatoria del servicio."
                ...
                {step === 4 && (
                  <motion.div key="step-4" {...pageTransition}>
                    <SectionCard
                      title="Vehículo y evidencia de la multa"
                      description="Selecciona el vehículo relacionado y, si lo deseas, adjunta imágenes de la citación para realizar lectura OCR automática."
                        ...
                        >
                          ...
                          >
                            ...
                            {vehiculosUsuario.map((v) => (
                              <option key={v.secuencial} value={v.secuencial}>
                                {v.descripcion}
                              </option>
                            ))}
                          </SelectInput>
                        ...
                        <div className="rounded-[24px] border border-slate-200 bg-slate-50/70 p-5">
                          <div className="mb-4 flex items-center justify-between gap-4">
                            <div>
                              ...
                              <p className="mt-1 text-sm text-slate-600">
                                {hayMultas
                                  ? "Opcional. Formatos permitidos: JPG, JPEG, PNG o PDF. Máximo 5 MB por archivo."
                                  : "Obligatorio (no hay multas registradas). Formatos permitidos: JPG, JPEG, PNG o PDF. Máximo 5 MB por archivo."}
                              </p>
                            </div>

                            {/* Botón "Omitir carga" solo si hay multas */}
                            {hayMultas && (
                            
								<button
								  type="button"
								  onClick={scrollToMultas}
								  className="inline-flex items-center gap-2 rounded-2xl border border-amber-200 bg-gradient-to-br from-white to-amber-50/70 px-5 py-2.5 text-sm font-semibold text-amber-800 shadow-sm transition-all duration-300 hover:scale-[1.02] hover:border-amber-300 hover:from-amber-50 hover:to-amber-100 hover:shadow-md hover:text-amber-900 focus:outline-none focus:ring-2 focus:ring-amber-400/50"
								>
								  <X className="h-4 w-4" />
								  Omitir carga
								</button>
                          ...
                          {isProcessing && (
                            <div className="mb-5 rounded-2xl border border-cyan-200 bg-cyan-50 p-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="h-2 flex-1 overflow-hidden rounded-full bg-cyan-100">
                                  <div
                              ...
                              <p className="mt-2 text-sm text-cyan-700">
                                Procesando OCR y extrayendo datos de la imagen...
                              </p>
                            </div>
                          )}
                          ...
                          <label className="group flex cursor-pointer flex-col items-center justify-center rounded-[24px] border-2 border-dashed border-slate-300 bg-white px-6 py-10 text-center transition hover:border-cyan-300 hover:bg-cyan-50/40">
                            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 transition group-hover:bg-cyan-100">
                        ...
                        {multasVehiculo.length > 0 && !cargandoMultas && (
                          <div ref={multasPanelRef} className="overflow-hidden rounded-[24px] border border-slate-200 bg-white">
                            <div className="border-b border-slate-200 px-5 py-4">
                              <h3 className="text-lg font-bold text-slate-900">
                                Multas detectadas para este vehículo
                              </h3>
                              <p className="mt-1 text-sm text-slate-600">
                                Puedes seleccionar una multa para precargar la información.
                              </p>
                            </div>
                              ...
                              <table className="min-w-full text-sm">
                                <thead className="bg-slate-50">
                                  <tr className="text-left text-slate-600">
                                    ...
                                    <th className="px-4 py-3">N° Infracción</th>
                                ...
                                <tbody className="divide-y divide-slate-200">
                                  {multasVehiculo.map((multa) => (
                                    <tr
                                      key={multa.secuencial}
                                      onClick={() => {
                                        setMultaSeleccionada(multa);
                                        setFormData((prev) => ({
                                          ...prev,
                                          numeroCitacion:
                                            multa.secuencia_1 || multa.id_factura,
                                          fechaCitacion:
                                            multa.fecha_emision.split(" ")[0],
                                        }));
                                      }}
                                      className={cn(
                                        "cursor-pointer transition hover:bg-slate-50",
                                        multaSeleccionada?.secuencial === multa.secuencial &&
                                          "bg-cyan-50"
                                      )}
                                    >
                                      <td className="px-4 py-3">
                                        <input
                                          type="radio"
                                          name="multaSeleccionada"
                                          checked={
                                            multaSeleccionada?.secuencial ===
                                            multa.secuencial
                                          }
                                          onChange={() => {}}
                                  ...
                                  {file.type.startsWith("image/") && file.preview ? (
                                    ...
                                    <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-slate-200 bg-slate-50">
                                      ...
                                      {file.type === "application/pdf" && (
                                        <span className="absolute mt-8 text-[10px] font-bold text-slate-500">PDF</span>
                                      )}
                                    </div>
                                  )}
                                      ...
                                      <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                        {formData.ocrResults[index].confidence >= 70 ? (
                                          <div className="flex items-center gap-2 text-sm font-medium text-emerald-700">
                                            ...
                                            {formData.ocrResults[index].confidence}%)
                                          </div>
                                        ) : (
                                        ...
                                        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                                            ...
                                            <div className="flex gap-2">
                                              <TextInput
                                                ...
                                                onChange={(value) =>
                                                  setFormData({
                                                    ...formData,
                                                    numeroCitacion: value,
                                                  })
                                                }
                                                error={errors.numeroCitacion}
                                              />
                                              <button
                                                type="button"
                                                onClick={() => {
                                          ...
                                          <div>
                                            ...
                                            <div className="flex gap-2">
                                              <input
                                                ...
                                                onChange={(e) =>
                                                  setFormData({
                                                    ...formData,
                                                    fechaCitacion: e.target.value,
                                                  })
                                                }
                                                className={cn(
                                                  "w-full rounded-2xl border bg-white px-4 py-3.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100 sm:text-[15px]",
                                                  errors.fechaCitacion ? "border-red-300" : "border-slate-200"
                                                )}
                                              />
                                              <button
                                                type="button"
                                            ...
                                            {errors.fechaCitacion && (
                                              <p className="mt-2 text-sm text-red-600">{errors.fechaCitacion}</p>
                                            )}
                                          </div>
                                        </div>
                                        ...
                                        <details className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                                          ...
                                          <div className="mt-3 max-h-48 overflow-y-auto whitespace-pre-wrap rounded-xl bg-slate-50 p-3 font-mono text-xs text-slate-700">
                          ...
                          multaSeleccionada && (
                            <div className="rounded-[24px] border border-slate-200 bg-white p-5">
                              <h4 className="text-lg font-bold text-slate-900">
                                Detalles de la multa seleccionada
                              </h4>
                              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                                <SummaryItem
                                  label="N° Infracción"
                                  value={
                                    multaSeleccionada.secuencia_1 ||
                                    multaSeleccionada.id_factura
                                  }
                                  icon={<Hash className="h-4 w-4" />}
                                />
                                <SummaryItem
                                  label="Ente"
                                  value={multaSeleccionada.ente}
                                  icon={<Shield className="h-4 w-4" />}
                                />
                                <SummaryItem
                                  label="Fecha emisión"
                                  value={multaSeleccionada.fecha_emision}
                                  icon={<Calendar className="h-4 w-4" />}
                                />
                                <SummaryItem
                                  label="Total"
                                  value={`$${multaSeleccionada.total.toFixed(2)}`}
                ...
                {step === 5 && (
                  <motion.div key="step-5" {...pageTransition}>
                    <SectionCard
                      title="Revisión final antes del análisis"
                      description="Revisa cuidadosamente toda la información antes de continuar con el diagnóstico jurídico generado por IA."
                      ...
                      <div className="space-y-6">
                            ...
                            <div className="rounded-[24px] border border-slate-200 bg-white p-5">
                              ...
                              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <SummaryItem
                                  ...
                                  value={
                                    vehiculosUsuario.find(
                                      (v) =>
                                        v.secuencial.toString() === formData.vehiculo
                                ...
                                <SummaryItem
                                  ...
                                  value={`${formData.archivos.length} archivo(s)`}
                        ...
                        <div className="rounded-[24px] border border-slate-200 bg-white p-5">
                          <div className="space-y-5">
                            <label className="flex items-start gap-3 rounded-2xl border border-slate-200 p-4 transition hover:bg-slate-50">
                              <input
                                type="checkbox"
                                ...
                                className="mt-1 h-4 w-4 rounded accent-cyan-600"
                              />
                              <div>
                                ...
                                <p className="mt-1 text-sm text-slate-600">
                                  Declaro que los datos y documentos cargados corresponden al caso real.
                                </p>
                            ...
                            <label className="flex items-start gap-3 rounded-2xl border border-slate-200 p-4 transition hover:bg-slate-50">
                              <input
                                type="checkbox"
                                checked={formData.aceptaTerminosUso}
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    aceptaTerminosUso: e.target.checked,
                                  })
                                }
                                className="mt-1 h-4 w-4 rounded accent-cyan-600"
                              />
                              <div>
                                <p className="text-sm font-semibold text-slate-900">
                                  He leído y acepto los términos y condiciones de NoPay Legal
                                </p>
                                <p className="mt-1 text-sm text-slate-600">
                                  Autorizo el tratamiento de datos y comprendo el alcance del servicio.
                                </p>
                                {errors.aceptaTerminosUso && (
                                  <p className="mt-2 flex items-center gap-2 text-sm text-red-600">
                                    <AlertCircle className="h-4 w-4" />
                                    {errors.aceptaTerminosUso}
                                  </p>
                ...
                {step === 6 && (
                  <motion.div key="step-6" {...pageTransition}>
                    {(cargandoIA || !respuestaIA) && !errorIA ? (
                      <SectionCard
                        title="Análisis jurídico automatizado"
                        description="Nuestra IA está revisando la información que has proporcionado para elaborar un diagnóstico legal claro y útil."
                      ...
                      >
                        <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[24px] border border-cyan-100 bg-gradient-to-br from-cyan-50 via-white to-blue-50 p-8 text-center">
                          <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-[28px] bg-white shadow-lg">
                          ...
                          <p className="mt-3 max-w-xl text-sm leading-7 text-slate-600 sm:text-base">
                            Estamos interpretando tus datos, evaluando la documentación adjunta y preparando un diagnóstico legal personalizado.
                          </p>

                          <div className="mt-6 w-full max-w-md overflow-hidden rounded-full bg-slate-200">
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{ width: "100%" }}
                              transition={{
                      ...
                      <SectionCard
                        title="Diagnóstico IA Jurídica"
                        description="Aquí tienes el análisis generado en base a la información que registraste."
                        icon={<ShieldCheck className="h-7 w-7" />}
                      >
                        {errorIA && (
                        ...
                        {htmlIA && !cargandoIA && (
                          <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
                            <div className="mb-6 border-b border-slate-200 pb-5 text-center">
                              <h3 className="text-2xl font-bold text-slate-900">
                                Diagnóstico Jurídico de tu Multa de Tránsito
                              </h3>
                            ...
                            <div
  id="documento-ia-juridica"
  className="
    max-w-none
    text-[15px] leading-[1.75] text-slate-700 sm:text-[15.5px]
    ...
    [&_p]:px-5
    [&_p]:py-4
    [&_p]:shadow-[0_10px_28px_rgba(15,23,42,0.04)]
    ...
    [&_li]:rounded-2xl
    [&_li]:border
    [&_li]:border-slate-100
    [&_li]:bg-white
    [&_li]:px-5
    [&_li]:py-4
    [&_li]:leading-7
    [&_li]:shadow-[0_10px_28px_rgba(15,23,42,0.04)]
...
/>
                ...
                {step === 7 && (
                  <motion.div key="step-7" {...pageTransition}>
                    <SectionCard
                      title="Protegiendo tu información"
                      description="Estamos preparando un entorno seguro para finalizar el proceso. No cierres esta ventana."
                      icon={<ShieldCheck className="h-7 w-7" />}
                    >
                        ...
                        {securityStage >= 0 && securityStage < securityMessages.length ? (
                          <>
                            <div className="relative mb-8 flex h-28 w-28 items-center justify-center rounded-full bg-white shadow-xl ring-4 ring-cyan-100">
                              {React.createElement(securityMessages[securityStage].icon, {
                                className: cn(
                                  "h-12 w-12",
                                  `text-${securityMessages[securityStage].color}-600`
                                ),
                              })}
                              <motion.div
                            ...
                            >
                              <p className="text-2xl font-bold text-slate-900">
                                {securityMessages[securityStage].text}
                              </p>
                              <p className="mt-2 text-sm text-slate-600">
                                {securityStage === securityMessages.length - 1
                            ...
                            <div className="mt-10 w-full max-w-md">
                              ...
                              <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200">
                                <motion.div
                                  ...
                                  initial={{ width: 0 }}
                                  animate={{ width: `${((securityStage + 1) / securityMessages.length) * 100}%` }}
                                  transition={{ duration: 0.3 }}
                                />
                              </div>
                            </div>
            ...
            <aside className="space-y-6">
              ...
              <div className="rounded-[28px] border border-slate-200/80 bg-white/90 p-5 shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <SummaryItem
                    label="Agencia"
                    value={formData.agencia || "Pendiente"}
                    icon={<Shield className="h-4 w-4" />}
                  />
                  <SummaryItem
                    label="Citación"
                    value={formData.numeroCitacion || "Pendiente"}
                    icon={<Hash className="h-4 w-4" />}
                  />
                  <SummaryItem
                    label="Archivos"
                    value={`${formData.archivos.length} adjunto(s)`}
                    icon={<Paperclip className="h-4 w-4" />}
                  />
                </div>
              </div>
              ...
              <div className="mx-auto flex max-w-7xl items-center justify-between rounded-[28px] border border-slate-200/80 bg-white/90 p-4 shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl">
                ...
                <div>
                  <button
                    onClick={nextStep}
                    type="button"
                    className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-600 to-blue-700 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-cyan-500/20 transition hover:translate-y-[-1px] hover:shadow-xl"
                  >
                    {step < 5 && "Siguiente"}
                    {step === 5 && "Ver análisis IA"}
                    {step === 6 && "Procesar y pagar"}
        ...
        {showDocsModal && (
          ...
          >
            <motion.div
              ...
              className="relative h-[80vh] w-full max-w-4xl overflow-hidden rounded-[28px] bg-white shadow-2xl"
            >
              <button
                onClick={() => setShowDocsModal(false)}
                className="absolute right-4 top-4 z-10 rounded-xl bg-white/90 p-2 text-slate-500 shadow transition hover:text-slate-800"
              >
                <X className="h-5 w-5" />
              </button>
              <iframe
